...
author: "Ben Cresswell edited by Charlotte Hyndman"
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, alpha = 3))
```
...
```{r}
?geom_smooth
#does not display confidence interval
```
...
# they will look the same because it is the same code
```

```{r}
ggplot() + 
  geom_point(data = mpg, mapping = aes(x = displ, y = hwy, colour = drv)) + 
  geom_smooth(data = mpg, mapping = aes(x = displ, y = hwy, linetype = drv, colour = drv))
```
...
Specified the y axis because we are not using the default stat. 
